#!/usr/bin/env python3
"""Build and operate REC-011 with OpenCart and complete Ecwid acquisition."""
...
import package_source_ready_generation as rec010
...
PackagedAsset = rec010.PackagedAsset
...
def default_assets() -> list[PackagedAsset]:
    ...
    return [
        PackagedAsset(
            project / "scripts/package_complete_source_generation.py",
            "tools/package_complete_source_generation.py",
            "recovery-tool",
        ),
        PackagedAsset(
            project / "scripts/capture_ecwid_api_v2.py",
            "tools/capture_ecwid_api_v2.py",
            "capture-tool",
        ),
        PackagedAsset(
            project / "docs/recovery/ecwid-api-complete-capture-v2.md",
            "docs/recovery/ecwid-api-complete-capture-v2.md",
            "capture-contract",
        ),
        PackagedAsset(
            project / "docs/recovery/complete-source-package-v1.md",
            "docs/recovery/complete-source-package-v1.md",
            "capture-contract",
        ),
    ]
    ...
    try:
        ...
        for asset in assets:
            ...
            if not source.is_file():
                raise ValueError(f"required REC-011 source asset is missing: {source.name}")
            ...
            connection.execute(
                ...
                (
                    ...
                    f"REC-011 packaged {asset.artifact_type}: {relative.name}",
                    f"rec011:{asset.artifact_type}:{relative.as_posix()}",
...
def stage_complete_source_readiness(
    root: Path,
    ...
    try:
        ...
        connection.close()
    manifest = rec010.rec009.rec008._remove_legacy_absolute_paths(manifest)
    ...
    source_capture.update(
        {
            ...
            "ecwid_contract": "docs/recovery/ecwid-api-complete-capture-v2.md",
            "captured_private_exports": False,
        }
    )
    manifest.setdefault("commerce_import", {})["tool"] = "tools/package_complete_source_generation.py"
...
def stage_and_import_bundle(root: Path, manifest_path: Path, schema_path: Path | None = None) -> dict:
    return rec010.stage_and_import_bundle(root, manifest_path, schema_path)
...
def create_generation(source: Path, destination: Path) -> dict:
    ...
    rec010.verify_generation(source, require_empty=True)
    temporary = destination.with_name(f".{destination.name}.building-{uuid.uuid4().hex}")
    try:
        shutil.copytree(source, temporary)
        stage_complete_source_readiness(temporary, default_assets())
...
def verify_generation(root: Path, require_empty: bool = False) -> dict:
    ...
    if manifest.get("generation") != GENERATION or manifest.get("parent_generation") != PARENT_GENERATION:
        raise ValueError("REC-011 generation lineage is invalid")
    absolute = rec010.rec009.rec008._manifest_absolute_paths(manifest)
    if absolute:
    ...
    if capture.get("readiness") != READINESS:
        raise ValueError("REC-011 source-capture readiness metadata is invalid")
    required = [
    ...
    for relative in required:
        if not relative or not (root / relative).is_file():
            raise ValueError(f"REC-011 packaged dependency is missing: {relative}")
    connection = sqlite3.connect(root / recovery_package.DATABASE_FILE)
    try:
        metadata = dict(connection.execute("SELECT key,value FROM recovery_metadata"))
        counts = {
            table: connection.execute(f"SELECT COUNT(*) FROM {table}").fetchone()[0]
            for table in rec010.rec009.rec008.EMPTY_IMPORT_TABLES
        }
    ...
    if metadata.get("generation") != GENERATION or metadata.get("parent_generation") != PARENT_GENERATION:
        raise ValueError("REC-011 database lineage metadata is invalid")
    if metadata.get("source_capture_readiness") != READINESS:
        raise ValueError("REC-011 database source-capture readiness is invalid")
    if require_empty and (sum(counts.values()) or import_sources):
        raise ValueError("fresh REC-011 contains source-backed commerce/import rows")
    return {
        "valid": True,
        "generation": GENERATION,
        "parent_generation": PARENT_GENERATION,
