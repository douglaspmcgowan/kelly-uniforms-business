...
MODULE_PATH = Path(__file__).with_name("package_source_ready_generation.py")


def load_packager():
    if not MODULE_PATH.is_file():
        raise AssertionError("package_source_ready_generation.py is missing")
    if str(MODULE_PATH.parent) not in sys.path:
        sys.path.insert(0, str(MODULE_PATH.parent))
    spec = importlib.util.spec_from_file_location("package_source_ready_generation", MODULE_PATH)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


class SourceReadyGenerationTests(unittest.TestCase):
    def make_rec009_fixture(self):
        ...
        connection.executemany(
            ...
            [("generation", "REC-009"), ("parent_generation", "REC-008")],
        )
        connection.commit()
        connection.close()
        ...
        tooling.mkdir()
        for name in ("opencart.py", "contract.md"):
            (tooling / name).write_text(f"{name}\n", encoding="utf-8")
        return temporary, root, tooling

    def test_stage_adds_opencart_capture_and_promotes_generation(self):
        packager = load_packager()
        temporary, root, tooling = self.make_rec009_fixture()
        self.addCleanup(temporary.cleanup)
        assets = [
            packager.PackagedAsset(
                tooling / "opencart.py", "tools/opencart.py", "capture-tool"
            ),
            packager.PackagedAsset(
        ...
        report = packager.stage_source_readiness(
            root, assets, captured_at="2026-08-10T03:00:00Z"
        )

        manifest = json.loads((root / "package-manifest.json").read_text(encoding="utf-8"))
        self.assertEqual(manifest["generation"], "REC-010")
        self.assertEqual(manifest["parent_generation"], "REC-009")
        self.assertEqual(
            manifest["source_capture"]["readiness"],
            "opencart-and-ecwid-tools-packaged-awaiting-authenticated-exports",
        )
        self.assertEqual(
            manifest["source_capture"]["opencart_tool"],
        ...
        try:
            ...
            connection.close()
        self.assertEqual(metadata["generation"], "REC-010")
        self.assertEqual(metadata["parent_generation"], "REC-009")
        self.assertEqual(
    ...
    def test_default_assets_include_opencart_capture_tool_and_runbook(self):
        packager = load_packager()

        destinations = {asset.destination for asset in packager.default_assets()}

        self.assertIn("tools/capture_opencart_native_export.py", destinations)
        self.assertIn("docs/recovery/opencart-native-capture-runbook.md", destinations)
        self.assertIn("tools/package_source_ready_generation.py", destinations)
