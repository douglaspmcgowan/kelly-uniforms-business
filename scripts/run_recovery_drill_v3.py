#!/usr/bin/env python3
"""Cache-free, generation-aware representative recovery drill."""
...
def cache_artifacts(root: Path) -> list[Path]:
    root = Path(root).resolve()
    found = {
        path
        for path in root.rglob("*")
        if path.name == "__pycache__" or (path.is_file() and path.suffix.lower() == ".pyc")
    }
    return sorted(found, key=lambda path: path.relative_to(root).as_posix())


def assert_no_cache_artifacts(root: Path) -> None:
    found = cache_artifacts(root)
    if found:
        relative = [path.relative_to(Path(root).resolve()).as_posix() for path in found[:3]]
        raise ValueError(f"authority contains Python cache artifacts: {relative}")


def authority_verifier(root: Path):
    root = Path(root).resolve()
    assert_no_cache_artifacts(root)
...
def run_drill(authority: Path, destination: Path, schema: Path | None = None) -> dict:
    ...
    assert_no_cache_artifacts(authority)
    ...
    try:
        ...
        marker = {
            ...
            "run_id": v1.RUN_ID,
            "promotable": False,
            "drill_version": "v3-cache-free-generation-aware",
        }
